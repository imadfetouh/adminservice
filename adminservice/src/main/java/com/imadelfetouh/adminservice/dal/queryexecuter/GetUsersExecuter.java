...
public class GetUsersExecuter implements QueryExecuter<List<UserDTO>> {
    ...
    public ResponseModel<List<UserDTO>> executeQuery(Session session) {
        ...
        Query query = session.createQuery("SELECT new com.imadelfetouh.adminservice.model.dto.UserDTO(u.userId, cast(u.role as string) , u.username, u.photo, u.profile.bio, u.profile.location, u.profile.website) FROM User u LEFT JOIN u.profile p");
